...
/**
 * @file components/dashboard/views/packages/PackagesList.tsx
 * @description Credit Packages view. Reads from DashboardDataContext cache.
 */

import { useState } from "react";
import { Edit3, Package, Zap } from "lucide-react";
...
import type { CreditPackageRecord } from "@/lib/types";

export default function PackagesList() {
  const { data: packages, isLoading, error, invalidate } = useDashboardData("packages");

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedPackage, setSelectedPackage] = useState<CreditPackageRecord | null>(null);

  if (isLoading) return <SystemLoader message="Loading credit packages..." />;
  if (error) return <DataErrorState message={error} onRetry={invalidate} />;

  const safePackages = packages ?? [];

  const handleEdit = (pkg: CreditPackageRecord) => {
              ...
              {safePackages.map((pkg) => (
                <div key={pkg.id} className="p-5 border border-border/50 rounded-xl bg-card flex flex-col gap-3 relative shadow-sm">
                  ...
                  <div className="absolute top-6 right-4">
                    <button onClick={() => handleEdit(pkg)} className="text-primary hover:opacity-70 p-1 cursor-pointer">
                  ...
                  {safePackages.map((pkg) => (
                    <tr key={pkg.id} className="hover:bg-muted/5 transition-colors bg-background">
                      ...
                      <td className="px-6 py-4 font-mono font-medium flex items-center gap-2">
                        <Zap className="w-4 h-4 text-amber-500" /> {pkg.credits} credits
                      </td>
                      <td className="px-6 py-4 text-right">
                        <Button variant="ghost" size="sm" onClick={() => handleEdit(pkg)} className="rounded-full hover:bg-primary/10 hover:text-primary cursor-pointer">
