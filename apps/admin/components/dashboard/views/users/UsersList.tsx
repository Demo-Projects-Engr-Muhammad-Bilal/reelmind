...
/**
 * @file components/dashboard/views/users/UsersList.tsx
 * @description User Directory view. Reads from DashboardDataContext cache.
 *
 * KEY REFACTOR — Cross-view navigation:
 * Previously used sessionStorage.setItem("audit_search", email) to pass state
 * to UsageLogsList. Now uses setCrossNavSearch() from DashboardProvider context.
 * Zero sessionStorage writes remain in this file.
 */

import React, { useEffect, useState } from "react";
import { Search, Download, ShieldAlert, ShieldCheck, Zap, Ban, CheckCircle2, Eye, ChevronLeft, ChevronRight, User as UserIcon, Calendar, Video, X, Coins, Snowflake, MessageCircle, Activity, Clapperboard, Loader2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Dialog, DialogContent, DialogTitle, DialogDescription } from "@/components/ui/dialog";
import { updateUserCreditsAction, toggleUserStatusAction } from "@/app/actions/user";
...
export default function UsersList() {
  const { setActiveView, setCrossNavSearch } = useDashboard();
  const { data: users, isLoading, error, invalidate } = useDashboardData("users");
  ...
  if (isLoading) return <SystemLoader message="Loading user directory..." />;
  if (error) return <DataErrorState message={error} onRetry={invalidate} />;

  const safeUsers = users ?? [];

  // ── Navigation helpers (replaces old sessionStorage writes) ──────────────
  const handleNavigateToAudit = (email: string) => {
    setCrossNavSearch("audit", email);
    setIsViewModalOpen(false);
    setActiveView("audit");
  };

  const handleNavigateToReels = (email: string) => {
    setCrossNavSearch("reels", email);
    setIsViewModalOpen(false);
    setActiveView("reels");
  };
  ...
  const filteredUsers = safeUsers.filter((user) => {
    ...
    const matchesFilter =
      ...
      (filterStatus === "Frozen" && !user.isVerified);
  ...
  // ── Action Handlers ───────────────────────────────────────────────────────
  const handleOpenCreditModal = (user: UserRecord) => {
  ...
  const handleUpdateCredits = async () => {
    ...
    if (res.success) {
      ...
      toast.error(res.error || "Failed to update credits.", { id: toastId });
    }
    setIsUpdating(false);
  };
                    ...
                    {paginatedUsers.map((user) => (
                      <tr key={user.id} className="hover:bg-muted/5 transition-colors bg-background">
                        ...
                        <td className="px-6 py-4 align-middle font-mono font-medium">
                          <div className="flex items-center gap-2"><Zap className="w-4 h-4 text-amber-500" /> {user.credits}</div>
                        </td>
                        <td className="px-6 py-4 align-middle text-right">
                          <div className="flex justify-end gap-2">
                            <Button variant="ghost" size="sm" onClick={() => { setUserToView(user); setIsViewModalOpen(true); }} className="rounded-full hover:bg-primary/10 hover:text-primary cursor-pointer"><Eye className="w-4 h-4" /></Button>
                            ...
                            <Button variant="outline" size="sm" onClick={() => { setUserToToggle(user); setIsFreezeModalOpen(true); }} className={`rounded-full border-border/50 cursor-pointer ${user.isVerified ? "hover:bg-destructive/10 hover:text-destructive" : "hover:bg-emerald-500/10 hover:text-emerald-500"}`}>
                  ...
                  <div key={user.id} className="p-5 border border-border/50 rounded-xl bg-background flex flex-col gap-3 relative shadow-sm">
                    <div className="flex justify-between items-start pr-12">
                      <div className="flex-1 overflow-hidden">
                        ...
                        <div className="mt-2">
                          {user.isVerified ? (
                            <span className="text-[10px] bg-emerald-500/10 text-emerald-500 px-2 py-0.5 rounded-full uppercase tracking-wider font-bold">Active</span>
                          ) : (
                    ...
                    <div className="flex items-center gap-1.5 bg-muted px-2.5 py-1 rounded-md border border-border/50 w-fit">
                    ...
                    <div className="absolute top-4 right-3 flex flex-col gap-2">
                      <button onClick={() => { setUserToView(user); setIsViewModalOpen(true); }} className="text-primary hover:opacity-70 p-2 bg-muted/50 rounded-md cursor-pointer"><Eye className="w-4 h-4" /></button>
                      ...
                      <button onClick={() => { setUserToToggle(user); setIsFreezeModalOpen(true); }} className={`${user.isVerified ? "text-destructive" : "text-emerald-500"} hover:opacity-70 p-2 bg-muted/50 rounded-md cursor-pointer`}>
        ...
        <div className="mt-auto p-4 border-t border-border/50 bg-muted/5 flex items-center justify-between shrink-0">
          ...
          {totalPages > 1 && (
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="icon" onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="w-8 h-8 rounded-full cursor-pointer"><ChevronRight className="w-4 h-4" /></Button>
            </div>
          )}
        </div>
      </div>
                ...
                <div className="flex items-center gap-3 sm:gap-4 pr-8 w-full">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-1.5">
                      <button onClick={() => handleNavigateToAudit(userToView.email)} className="flex items-center gap-1.5 text-[10px] sm:text-xs font-semibold text-primary hover:opacity-70 transition-opacity cursor-pointer">
                        <Activity className="w-3 h-3" /> View Usage Logs
                      </button>
                      <button onClick={() => handleNavigateToReels(userToView.email)} className="flex items-center gap-1.5 text-[10px] sm:text-xs font-semibold text-primary hover:opacity-70 transition-opacity cursor-pointer">
                        <Clapperboard className="w-3 h-3" /> View Reels
                      </button>
                    </div>
                  </div>
                </div>
                <Button variant="ghost" size="icon" onClick={() => setIsViewModalOpen(false)} className="absolute top-3 right-3 sm:top-4 sm:right-4 rounded-full cursor-pointer h-8 w-8 text-muted-foreground hover:text-foreground hover:bg-muted">
                  <X className="w-4 h-4" />
                </Button>
              </div>
              ...
              <div className="px-4 sm:px-6 pb-4 sm:pb-6 space-y-3 sm:space-y-4">
                ...
                {userToView.telegramCreds ? (
                  <div className="grid grid-cols-[80px_1fr] sm:grid-cols-[100px_1fr] gap-y-2 sm:gap-y-3 text-xs sm:text-sm">
                    ...
                    <div className="font-mono text-[10px] sm:text-xs mt-0.5 text-foreground truncate pr-2">{userToView.telegramCreds.chatId}</div>
                  </div>
                ) : (
      ...
      <Dialog open={isFreezeModalOpen} onOpenChange={setIsFreezeModalOpen}>
        <DialogContent className="w-[95vw] max-w-[500px] p-6 rounded-2xl bg-background border-border/50 [&>button]:hidden">
          ...
          <DialogDescription className="text-center text-base text-muted-foreground mt-2">
            Are you sure you want to {userToToggle?.isVerified ? "block" : "unblock"} <span className="font-semibold text-foreground">{userToToggle?.name || userToToggle?.email}</span>?
          ...
          <div className="flex justify-center gap-4 mt-8 w-full">
            <Button variant="outline" onClick={() => setIsFreezeModalOpen(false)} className="rounded-full flex-1 cursor-pointer"><X className="size-4 mr-2" />Cancel</Button>
            <Button variant="destructive" onClick={confirmToggleStatus} disabled={isToggling} className="rounded-full flex-1 cursor-pointer">
              {isToggling ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : (userToToggle?.isVerified ? <Snowflake className="w-4 h-4 mr-2" /> : <Zap className="w-4 h-4 mr-2" />)}
